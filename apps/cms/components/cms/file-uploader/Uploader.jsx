...
export function Uploader({ value, onChange, onUploadComplete, uploadEndpoint = '/api/admin/upload' }) {
  ...
  return (
    <Card {...getRootProps()} className={cn('relative border-2 border-dashed w-full', isDragActive ? 'border-primary bg-primary/10' : 'border-border')}>
      <CardContent className="flex items-center justify-center w-full p-4 min-h-[200px]">
        <input {...getInputProps()} />
        {uploading ? (
