    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Clerk requires email or phone number, not username
          // Use identifier if provided (email or phone), otherwise use email or phoneNumber
          if (lookupData.identifier) {
            ...
            // If no email or phone found, show error - Clerk requires email/phone
          ...
          const errorMessage = data?.error || data?.message || data?.data?.error || 'User not found';
      ...
      // Validate identifier before attempting sign-in
      if (!identifier || (identifier === username && !identifier.includes('@') && !identifier.match(/^\+?[1-9]\d{1,14}$/))) {
        setError('Invalid username. Please use your email address to sign in.');
        setLoading(false);
        return;
      }
