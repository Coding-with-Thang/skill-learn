...
const SignInPage = () => {
    ...
    try {
          ...
          if (response.ok) {
            ...
            if (lookupData.useUsername && lookupData.identifier === username) {
              ...
              try {
                ...
                if (!identifier) {
                  const errorMsg = t("unableToAuthenticate");
                  setError(errorMsg);
                  setLoading(false);
                  return;
                }
              ...
              const errorMsg = t("unableToFindEmail");
      ...
      if (result && result.status === 'complete' && result.createdSessionId) {
        ...
        if (totpFactor) {
          ...
          setError('');
          toast.info(t("pleaseEnterAuthenticatorCode"));
          setLoading(false);
          return;
        }
        ...
        if (phoneFactor) {
          try {
            ...
            setError('');
            toast.info(t("pleaseEnterPhoneCode"));
            setLoading(false);
            return;
          } catch (prepError: unknown) {
            const pe = prepError as { errors?: { message?: string }[]; message?: string };
            const errorMsg = pe.errors?.[0]?.message || pe.message || t("unableToSendVerificationCode");
            setError(errorMsg);
            setLoading(false);
            return;
          }
        ...
        if (emailFactor) {
          try {
            ...
            setError('');
            toast.info(t("pleaseEnterEmailCode"));
            setLoading(false);
            return;
          } catch (prepError: unknown) {
            const pe = prepError as { errors?: { message?: string }[]; message?: string };
  ...
  const handleSocialSignIn = async (strategy) => {
    ...
    try {
      ...
      const errorMessage = e.errors?.[0]?.message || t("failedToSignInSocial");
      setError(errorMessage);
      setLoading(false);
    }
  };
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      const e = err as { errors?: { message?: string }[] };
      const errorMessage = e.errors?.[0]?.message || t("failedToSendResetCode");
  ...
  const handlePasswordReset = async (e) => {
    ...
    try {
      ...
      const e = err as { errors?: { message?: string }[] };
      const errorMessage = e.errors?.[0]?.message || t("failedToResetPassword");
  ...
  const handleTwoFactor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = e.errors?.[0]?.message || e.message || t("invalidAuthCode");
      ...
      <div className="hidden lg:flex lg:w-[45%] relative bg-[#0F172A] overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between h-full p-12">
          ...
          <div className="space-y-6">
            ...
            >
              {t("readyToLevelUp")} <br />
              <span className="text-indigo-400">{t("levelUp")}</span> <br />
              {t("yourSkills")}
            </motion.h2>
            <motion.p
              initial={{ y: 30, opacity: 0 }}
          ...
          >
            ...
            <p className="text-sm text-slate-300 font-medium">
              {t("joinStudents")} <span className="text-white font-bold">12k+</span> {t("studentsToday")}
            </p>
          </motion.div>
        </div>
      </div>
            ...
            {!signInLoaded ? (
              ...
              <form onSubmit={handleTwoFactor} className="space-y-6">
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-bold text-slate-900 mb-2">{t("twoFactorAuth")}</h2>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">{t("authenticationCode")}</label>
              ...
              !resetEmail ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleForgotPassword(); }} className="space-y-6">
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-900 mb-2">{t("resetPassword")}</h2>
                    <p className="text-sm text-slate-600">
                ...
                <form onSubmit={handlePasswordReset} className="space-y-6">
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-900 mb-2">{t("resetPassword")}</h2>
                    <p className="text-sm text-slate-600">
                      {t("enterCodeAndNewPassword", { email: resetEmail })}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-slate-700 ml-1">{t("verificationCode")}</label>
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-indigo-500 transition-colors" />
                      <input
                        type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-slate-700 ml-1">{t("confirmNewPassword")}</label>
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-indigo-500 transition-colors" />
                      <input
                        type={showPassword ? 'text' : 'password'}
              ...
              <form onSubmit={handleSignIn} className="space-y-6">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-400">
                  ...
                  <span className="font-bold uppercase tracking-widest">{t("or")}</span>
                  <div className="h-px flex-1 bg-slate-100" />
                </div>
      ...
      <div className="fixed left-0 right-0 bottom-0 bg-transparent pointer-events-none">
        <div className="max-w-7xl mx-auto px-12 py-6 flex items-center justify-between pointer-events-auto">
          ...
          <div className="flex items-center gap-8">
            {[t("privacy"), t("terms"), t("legal")].map((link) => (
