...
export default function UserBadge() {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 pointer-events-none z-200 bg-[url('/chalkboard_texture.png')] bg-repeat opacity-15 mix-blend-screen"
        aria-hidden="true"
      />
      <div className="relative z-10 flex flex-col items-center w-full">
        ...
        {!isLoading ? (
          <div
            className="mt-10 px-4 sm:px-10 bg-white/5 backdrop-blur-md grid grid-cols-1 sm:grid-cols-3 gap-6 rounded-xl shadow-xl ring-1 ring-white/10 w-full max-w-3xl transition-all border-2 border-white/30"
            style={{
              boxShadow:
                "0 2px 16px 0 var(--card-shadow), 0 0 0 2px var(--ring)",
              fontFamily:
                "'Permanent Marker', 'Schoolbell', cursive, sans-serif",
            }}
          >
            <div className="p-4 flex flex-col justify-center items-center">
              <p className="font-bold text-4xl text-primary drop-shadow-sm">
                {formatNumber(points)}
              </p>
      ...
      <div
        className="absolute left-1/2 -translate-x-1/2 bottom-0 z-2000 w-4/5 h-9 bg-linear-to-r from-yellow-600 to-yellow-300 rounded-b-[18px] shadow-lg border-t-4 border-border flex items-center justify-end pr-8"
        aria-hidden="true"
      >
