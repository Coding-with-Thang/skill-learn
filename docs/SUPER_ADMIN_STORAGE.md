...
Body: { "userId": "user_xxx" } or { "email": "user@example.com" }
```

This endpoint:
- Updates Clerk's `publicMetadata.role = "super_admin"`
- Ensures user exists in database
- Can only be called by existing super admins
...
- **Access**: Set via Clerk Dashboard or `/api/admin/approve-user` API
