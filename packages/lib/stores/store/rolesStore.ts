...
export const useRolesStore = create((set, get) => ({
    ...
    return requestDeduplicator.dedupe(
      ...
      async () => {
        ...
        try {
          ...
          set({
            error: parseApiError(error) || "Failed to fetch roles",
            isLoading: false,
            retryCount: 0,
          });
          throw error;
    ...
    return requestDeduplicator.dedupe(
      ...
      async () => {
        ...
        try {
          ...
          set({
            error: parseApiError(error) || "Failed to fetch templates",
            isLoading: false,
          });
          throw error;
        }
  ...
  createRole: async (roleData, tenantId = null) => {
    ...
    try {
      ...
      set({
        error: parseApiError(error) || "Failed to create role",
        isLoading: false,
      });
      throw error;
    }
  ...
  updateRole: async (roleId, roleData, tenantId = null) => {
    ...
    try {
      ...
      set({
        error: parseApiError(error) || "Failed to update role",
        isLoading: false,
      });
      throw error;
    }
  ...
  deleteRole: async (roleId, tenantId = null) => {
    ...
    try {
      ...
      set({
        error: parseApiError(error) || "Failed to delete role",
        isLoading: false,
      });
      throw error;
    }
